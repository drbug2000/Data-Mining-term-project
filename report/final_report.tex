...
exceeding the provided baseline ($0.0436$) by $+154\%$. 
For Task 2, we represent queries, advertisements, and users in a shared text embedding space and maintain multiple per-user interest vectors to capture diverse user preferences. The model updates these interest vectors using both dense search-query signals and sparse but stronger clicked-ad signals, then ranks candidates by combining query–advertisement relevance with user–advertisement preference through an adaptive mixing weight.
This lightweight multi-interest retrieval model reaches NDCG@3 $=0.1538$, improving over a text-similarity-only baseline by
$+56\%$. We choose deterministic Task~1 components (weights, $K$, and threshold)
...
(Figure~\ref{fig:content-head}).
Formally, let $U,V\in\mathbb{R}^{384\times128}$ be learned projection matrices
and $\tau,b$ learned scalars. For query embedding $x_s$ and ad embedding $z_a$,
\begin{equation}
  \mathrm{logit}(s,a)=\tau\cdot\cos\!\bigl(Ux_s,\,Vz_a\bigr)+b,
  \label{eq:content}
\end{equation}
where each projected vector is L2-normalised before the dot product.
An ensemble of 30 independently seeded heads averages the logits to reduce
variance. We then combine the content head with trusted
...
sensitive. Second, most of the other $K$ yield external F1 ${=}0.1064$, within the
