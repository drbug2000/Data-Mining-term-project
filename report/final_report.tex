...
this reaches NDCG@3 $=0.1228$, improving over a text-similarity-only baseline by
$+24\%$. All model selection uses only an internal split of the training data;
...
clicked ads of its $K{=}100$ most similar queries is $0.884$, versus $0.677$ for
random ads (Figure~\ref{fig:knn}, Cohen's $d=1.27$). End to end, replacing the
direct similarity (Cohen's $d=0.04$) by SKNCP yields an $11\times$ stronger
click signal (Cohen's $d=0.44$).

\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{fig_smoothness.png}
\caption{Smoothness assumption. As query--query embedding similarity increases,
the similarity between the ads those queries clicked increases monotonically
($r=0.336$; the reverse direction gives $r=0.341$).}
\Description{Bar chart: clicked-ad similarity rises across query-similarity bins.}
\label{fig:smoothness}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.66\columnwidth]{fig_knn_validity.png}
\caption{K-NN signal validity. The clicked ad is far more similar to the clicked
ads of similar queries (0.884) than to random ads (0.677), Cohen's $d=1.27$.}
\Description{Two bars showing SKNCP similarity 0.884 vs random 0.677.}
\label{fig:knn}
\end{figure}
...
(SKNCP carries the largest weight, $3.0$). The neighborhood size $K{=}200$ and
the blend weight $\alpha{=}0.2$ are fixed a-priori constants (not tuned on
...
interest vector (long-term preference), combined by a weighted sum. Cold users
without click history fall back to query--ad similarity until enough behavioral
...
the multi-interest term yields a further $+24\%$ relative improvement
($0.0989\!\to\!0.1228$), showing that personalized preference provides
complementary signal beyond immediate query intent.
...
$=0.1228$, a $+24\%$ improvement over a strong text-similarity baseline. Across
