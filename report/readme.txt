AI506 Data Mining and Search — Term Project
Team: Sangjune Kim, Gyuchan An, Minseo Kang (KAIST)

================================================================================
HELP RECEIVED
================================================================================

We received no help from friends, classmates, lab TAs, or course staff beyond
the publicly posted lecture slides and the provided dataset.

Large language models (ChatGPT, Claude) were used for LaTeX editing, wording
refinement in the report, and minor Python syntax questions. All experimental
design, modeling decisions, numerical results, and final submissions were
produced and verified by the team.

================================================================================
COMMENT / EVALUATION
================================================================================

We found this project well-designed: having two related but methodologically
distinct tasks forced us to think carefully about when text similarity is and
is not an informative signal. The key insight — that direct query-ad cosine
barely separates clicks among displayed impressions (Cohen's d ≈ 0.11) but
strongly separates clicked from random ads (d ≈ 1.56) — only became clear
after careful EDA, and it drove the two-track design. The main difficulty was
the extreme label sparsity on Task 1 (1.11% CTR, 229 positives on validation),
which makes F1 estimates very noisy and hyper-parameter selection unreliable
without a stable proxy signal (we used Cohen's d on the internal split).

================================================================================
VALIDATION RESULTS
================================================================================

Task 1 (Click Prediction):
  External validation F1 (clicked-class) : 0.1109
  External validation AUC                : 0.713
  Bootstrap mean F1 (2000 resamples)     : 0.104
  Bootstrap 95% CI                       : [0.067, 0.144]
  Baseline provided (HistCTR > 0.01)     : 0.0436
  Relative improvement                   : +154%

Task 2 (Ad Prediction):
  Validation NDCG@3                      : 0.1398
  Query-only baseline NDCG@3             : 0.0989
  Relative improvement                   : +41%

================================================================================
HOW TO RUN
================================================================================

Requirements
------------
Python 3.10, PyTorch >= 2.0, numpy, pandas, sentence-transformers (for
embedding inspection only — embeddings are precomputed in .npy files).

Dataset layout (../datasets/ relative to project root, or adjust DATASET_DIR):
  searchinfo.csv, searchinfo_text_embs.npy
  adinfo.csv, adinfo_title_embs.npy
  userinfo.csv
  search_stream_training.csv
  click_validation_query.csv, click_validation_answer.csv
  ad_validation_query.csv, ad_validation_answer.csv
  click_test_query.csv, ad_test_query.csv

Step 0 — set up Python path
---------------------------
All scripts must be run from the project root so that the `shared/` and
`models/` packages resolve correctly:

  cd <project_root>

Step 1 — Task 1: reproduce click_test_answer.csv
-------------------------------------------------
The test prediction script trains a 30-seed content-head ensemble (MLP
projection, identical to the submitted model) and applies the full
SKNCP-boosted pipeline with frozen weights:

  conda run -n <env> python -X utf8 \
      models/m06_skncp/experiments/generate_click_test_answer.py

Output: click_test_answer.csv (columns: SearchID, AdID, IsClick)
Runtime: ~3 minutes on an NVIDIA A100 GPU; ~30 minutes on CPU.

To reproduce the exact validation score (F1 0.1109) using the frozen
content-score cache included in the repo:

  conda run -n <env> python -X utf8 \
      models/m06_skncp/experiments/task1_skncp_boosted.py

The frozen cache is at models/m06_skncp/results/cs_cache.npz.

Step 2 — Task 1: K sweep experiment (Table 2 in report)
---------------------------------------------------------
  conda run -n <env> python -X utf8 \
      models/m06_skncp/experiments/k_sweep_eval.py

Step 3 — Task 2: reproduce ad_test_answer.csv
----------------------------------------------
See models/m01_interest/experiments/tiered_gamma_eval.py for the final
3-tier adaptive model. The test answer was generated by applying the
trained multi-interest model to ds.test_ad_queries() and ranking all
17,518 ad candidates by score.

Step 4 — EDA figures (Figures 1 and 2 in report)
--------------------------------------------------
  conda run -n <env> python -X utf8 \
      models/m06_skncp/experiments/task1_skncp_model6.py

Produces the smoothness-assumption scatter (evidence_1.png) and KNN
signal validity histograms (evidence_2.png) in report/.

================================================================================
LABOR DIVISION
================================================================================

- Dataset audit and EDA: Sangjune Kim, Gyuchan An, Minseo Kang
- Task 1 SKNCP model (motivation, feature design, F1-fit, validation): Gyuchan An
- Task 2 multi-interest model (design, streaming update, validation): Sangjune Kim
- Report integration, LaTeX formatting, consistency checks: Minseo Kang
